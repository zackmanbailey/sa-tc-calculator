...
import logging, traceback, html, fcntl
...
# Security hardening imports (rate limiting, CSRF, headers, cookie settings)
try:
    from auth.security import (
        apply_security_headers, check_register_rate,
        SECURE_COOKIE_KWARGS, check_login_rate as sec_check_login_rate,
    )
    HAS_SECURITY = True
except ImportError:
    HAS_SECURITY = False
...
def _ensure_users_file():
    ...
    SEC-002: Generates random passwords on first run instead of hardcoded ones."""
    if os.path.isfile(USERS_PATH):
        return
    # Generate random passwords for first-run default users
    admin_password = secrets.token_urlsafe(12)
    brad_password = secrets.token_urlsafe(12)
    users = {
        "admin": {
        ...
        "brad": {
            "password": hash_password(brad_password),
    ...
    print(f"  [AUTH] Default login  →  admin / {admin_password}")
    print(f"  [AUTH]                →  brad  / {brad_password}")
    print(f"  [AUTH] *** SAVE THESE PASSWORDS — they are randomly generated and shown only once ***")

def verify_password(stored_hash: str, password: str) -> bool:
    """Verify password against stored hash (bcrypt, PBKDF2, or legacy SHA256)."""
    if HAS_BCRYPT and stored_hash.startswith("$2"):
        return bcrypt.checkpw(password.encode(), stored_hash.encode())
    # SEC-003: PBKDF2 format: "pbkdf2$salt_hex$hash_hex"
    if stored_hash.startswith("pbkdf2$"):
        parts = stored_hash.split("$")
        if len(parts) == 3:
            salt = bytes.fromhex(parts[1])
            expected = parts[2]
            computed = hashlib.pbkdf2_hmac("sha256", password.encode(), salt, 260000).hex()
            return secrets.compare_digest(computed, expected)
    # Legacy SHA-256 fallback (read-only, new hashes use PBKDF2)
...
def hash_password(password: str) -> str:
    ...
    # SEC-003: Use PBKDF2 with random salt instead of bare SHA-256
    salt = secrets.token_bytes(16)
    hashed = hashlib.pbkdf2_hmac("sha256", password.encode(), salt, 260000).hex()
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def set_default_headers(self):
        """Set security-related default headers including CORS and security hardening."""
        ...
        # SEC-011: Apply security headers (CSP, X-Frame-Options, etc.)
        if HAS_SECURITY:
    ...
    def check_xsrf_cookie(self):
        """SEC-001: CSRF check override — log-only for now until forms send XSRF tokens.
        TODO: Remove this override once all forms/AJAX include the _xsrf token,
        then enable xsrf_cookies=True in the Application constructor."""
        pass
    ...
    def prepare(self):
        ...
        if not AUTH_ENABLED:
            ...
            if not self.get_secure_cookie("sa_user"):
                _cookie_kw = SECURE_COOKIE_KWARGS if HAS_SECURITY else {"httponly": True, "samesite": "Lax"}
                self.set_secure_cookie("sa_user", "admin", expires_days=1, **_cookie_kw)
            return
    ...
    def post(self):
        try:
            ...
            self.set_secure_cookie("sa_user", username,
                                   expires_days=COOKIE_MAX_AGE_HOURS / 24.0, **_cookie_kw)
...
class RegisterSubmitHandler(BaseHandler):
    ...
    def post(self):
        try:
            # SEC-013: Rate limit registration attempts
            if HAS_SECURITY:
                ip = self.request.remote_ip or "unknown"
...
class LogoutHandler(BaseHandler):
    """GET|POST /auth/logout — Clear session and redirect to login.
    SEC-012: POST method added so logout can be triggered via form submission."""
    def _do_logout(self):
        try:
...
class InventorySaveHandler(BaseHandler):
    ...
    def post(self):
        try:
            body = json_decode(self.request.body)
            # Validate basic schema: must be a dict with "coils" as a dict
            if not isinstance(body, dict):
                self.set_status(400)
                self.set_header("Content-Type", "application/json")
                self.write(json_encode({"error": "Request body must be a JSON object"}))
                return
            if "coils" in body and not isinstance(body.get("coils"), dict):
                self.set_status(400)
                self.set_header("Content-Type", "application/json")
                self.write(json_encode({"error": "'coils' must be a JSON object/dict"}))
                return
            # Create backup of existing inventory before overwriting
            if os.path.isfile(INVENTORY_PATH):
                backup_path = INVENTORY_PATH + ".bak"
                try:
...
class InventoryAllocateHandler(BaseHandler):
    ...
    def post(self):
        try:
            ...
            # Use file locking to prevent race conditions on allocation
            lock_path = os.path.join(DATA_DIR, ".inventory.lock")
            with open(lock_path, 'w') as lock_file:
                ...
                if os.path.isfile(alloc_path):
                    with open(alloc_path) as f:
                        allocs = json.load(f)
                # Extract max existing allocation number to avoid ID collision (DATA-007)
                max_num = 0
                for a in allocs:
                    ...
                    if aid.startswith("ALLOC-"):
                        try:
                            ...
                            pass
                alloc_id = f"ALLOC-{max_num+1:04d}"
                allocs.append({
...
class ProjectStatusHandler(BaseHandler):
    ...
    def post(self):
        try:
            ...
            # Prevent backward stage transitions unless user is admin
            if old_stage and old_stage in PROJECT_STAGES and stage in PROJECT_STAGES:
                old_idx = PROJECT_STAGES.index(old_stage)
                new_idx = PROJECT_STAGES.index(stage)
                if new_idx < old_idx:
                    user_role = self.get_user_role() if hasattr(self, 'get_user_role') else ""
                    if user_role != "admin":
                        self.write(json_encode({
                            "ok": False,
                            "error": f"Cannot move project backward from '{old_stage}' to '{stage}'. Admin role required."
                        }))
                        return
...
class ProjectPageHandler(BaseHandler):
    ...
    def get(self, job_code):
        try:
            ...
            if not os.path.isfile(meta_path):
                self.set_status(404)
                # SEC-009: Escape job_code to prevent reflected XSS
                self.write(f"<h2>Project '{html.escape(job_code)}' not found</h2><a href='/'>Back to Dashboard</a>")
                return
...
class WorkOrderTransitionHandler(BaseHandler):
    ...
    def post(self):
        try:
            ...
            if not transition_item_status(item, new_status):
                self.set_status(400)
...
class WorkOrderQCPhotoHandler(BaseHandler):
    ...
    def post(self):
        try:
            ...
            # SEC-007: Sanitize job_code to prevent path traversal
            safe_job = re.sub(r"[^a-zA-Z0-9_-]", "_", job_code)
            # Save photo to qc_photos directory
            photo_dir = os.path.join(SHOP_DRAWINGS_DIR, safe_job, "qc_photos")
            os.makedirs(photo_dir, exist_ok=True)

            # Generate unique filename
            safe_item = re.sub(r"[^A-Za-z0-9_-]", "_", item_id)
            ext = os.path.splitext(filename)[1].lower() or ".jpg"
            # SEC-007: Whitelist allowed image extensions
            allowed_extensions = {".jpg", ".jpeg", ".png", ".gif", ".webp"}
            if ext not in allowed_extensions:
                self.write(json_encode({"ok": False, "error": f"File type '{ext}' not allowed. Use jpg, png, gif, or webp."}))
                return
...
class LiveUpdateWebSocket(tornado.websocket.WebSocketHandler):
    ...
    def check_origin(self, origin):
        """SEC-005: Validate WebSocket origin against the request host."""
        try:
            from urllib.parse import urlparse
            parsed = urlparse(origin)
            host = self.request.headers.get("Host", "")
            # Allow if origin host matches request host (ignoring port for localhost)
            return parsed.hostname == host.split(":")[0]
        except Exception:
            return False

    def open(self):
        """SEC-006: Check authentication before allowing WebSocket connection."""
        if AUTH_ENABLED:
            cookie = self.get_secure_cookie("sa_user")
            if not cookie:
                logger.warning("WebSocket rejected — no auth cookie from %s", self.request.remote_ip)
...
# ─────────────────────────────────────────────────────────────
# NEW API STUB HANDLERS — Missing endpoints
# ─────────────────────────────────────────────────────────────

class ShopFloorSummaryHandler(BaseHandler):
    """GET /api/shop-floor/summary — Shop floor summary data."""
    required_roles = ["admin", "estimator", "shop"]
        ...
        try:
            ...
            try:
                ...
                if _os.path.isdir(SHOP_DRAWINGS_DIR):
                    from shop_drawings.work_orders import list_all_work_orders
                    all_wos = list_all_work_orders(SHOP_DRAWINGS_DIR)
                    active_orders = sum(1 for wo in all_wos if wo.get("status") in ("in_progress", "approved", "queued"))
            except Exception:
                pass
            self.write(json_encode({
                "ok": True, "machines": machines,
                "active_orders": active_orders, "summary": {}
            }))
        except Exception as e:
            logger.error(f"ShopFloorSummaryHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class WorkStationScanHandler(BaseHandler):
    """POST /api/work-station/scan — Scan QR at work station."""
    required_roles = ["admin", "estimator", "shop"]
    ...
    def post(self):
        try:
            ...
            if not qr_code:
                self.set_status(400)
                self.write(json_encode({"ok": False, "error": "qr_code required"}))
                return
            item = {}
            try:
                from shop_drawings.work_orders import scan_qr_code
                result = scan_qr_code(SHOP_DRAWINGS_DIR, qr_code, station)
                if result:
                    item = result
            except Exception:
                pass
            self.write(json_encode({"ok": True, "status": "ok", "item": item, "station": station}))
        except Exception as e:
            logger.error(f"WorkStationScanHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class ScheduleSummaryHandler(BaseHandler):
    """GET /api/schedule/summary — Production schedule summary."""
    required_roles = ["admin", "estimator", "shop"]
    ...
    def get(self):
        try:
            self.set_header("Content-Type", "application/json")
            self.write(json_encode({
                "ok": True, "total_orders": 0, "on_track": 0,
                "delayed": 0, "capacity_pct": 0
            }))
        except Exception as e:
            logger.error(f"ScheduleSummaryHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class ScheduleCapacityHandler(BaseHandler):
    """GET /api/schedule/capacity/usage and /api/schedule/capacity/forecast."""
    required_roles = ["admin", "estimator", "shop"]

    def get(self):
        try:
            self.set_header("Content-Type", "application/json")
            self.write(json_encode({"ok": True, "usage": [], "forecast": []}))
        except Exception as e:
            logger.error(f"ScheduleCapacityHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class ScheduleBottlenecksHandler(BaseHandler):
    """GET /api/schedule/bottlenecks — Identify production bottlenecks."""
    required_roles = ["admin", "estimator", "shop"]

    def get(self):
        try:
            self.set_header("Content-Type", "application/json")
            self.write(json_encode({"ok": True, "bottlenecks": []}))
        except Exception as e:
            logger.error(f"ScheduleBottlenecksHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class ScheduleOverdueHandler(BaseHandler):
    """GET /api/schedule/overdue — List overdue schedule items."""
    required_roles = ["admin", "estimator", "shop"]

    def get(self):
        try:
            self.set_header("Content-Type", "application/json")
            self.write(json_encode({"ok": True, "overdue_items": []}))
        except Exception as e:
            logger.error(f"ScheduleOverdueHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class FieldSummaryAPIHandler(BaseHandler):
    """GET /api/field/summary — Field operations summary."""
    required_roles = ["admin", "estimator", "shop"]
    ...
    def get(self):
        try:
            self.set_header("Content-Type", "application/json")
            self.write(json_encode({
                "ok": True, "active_installs": 0, "crews": [], "projects": []
            }))
        except Exception as e:
            logger.error(f"FieldSummaryAPIHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class FieldProjectCompletionAPIHandler(BaseHandler):
    """GET /api/field/project-completion — Project completion status."""
    required_roles = ["admin", "estimator", "shop"]

    def get(self):
        try:
            self.set_header("Content-Type", "application/json")
            self.write(json_encode({"ok": True, "projects": []}))
        except Exception as e:
            logger.error(f"FieldProjectCompletionAPIHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class FieldPunchListAPIHandler(BaseHandler):
    """GET/POST /api/field/punch-list — Punch list items."""
    required_roles = ["admin", "estimator", "shop"]
    ...
    def get(self):
        try:
            ...
            items = []
            try:
                punch_path = os.path.join(DATA_DIR, "field_punch_list.json")
                if os.path.isfile(punch_path):
                    with open(punch_path) as f:
                        items = json.load(f)
            except Exception:
                pass
            self.write(json_encode({"ok": True, "items": items}))
        except Exception as e:
            logger.error(f"FieldPunchListAPIHandler GET error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))
    ...
    def post(self):
        try:
            ...
            with open(punch_path, "w") as f:
                json.dump(items, f, indent=2)
            self.write(json_encode({"ok": True, "item": new_item}))
        except Exception as e:
            logger.error(f"FieldPunchListAPIHandler POST error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class FieldInstallationsAPIHandler(BaseHandler):
    """GET /api/field/installations — Field installation records."""
    required_roles = ["admin", "estimator", "shop"]

    def get(self):
        try:
            self.set_header("Content-Type", "application/json")
            self.write(json_encode({"ok": True, "installations": []}))
        except Exception as e:
            logger.error(f"FieldInstallationsAPIHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class FieldDailyReportsAPIHandler(BaseHandler):
    """GET/POST /api/field/daily-reports — Field daily reports."""
    required_roles = ["admin", "estimator", "shop"]
    ...
    def get(self):
        try:
            ...
            reports = []
            try:
                reports_path = os.path.join(DATA_DIR, "field_daily_reports.json")
            ...
            logger.error(f"FieldDailyReportsAPIHandler GET error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))
    ...
    def post(self):
        try:
            ...
            with open(reports_path, "w") as f:
                json.dump(reports, f, indent=2)
            self.write(json_encode({"ok": True, "report": new_report}))
        except Exception as e:
            logger.error(f"FieldDailyReportsAPIHandler POST error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class FieldConfirmInstallAPIHandler(BaseHandler):
    """POST /api/field/confirm-install — Confirm field installation."""
    required_roles = ["admin", "estimator", "shop"]
    ...
    def post(self):
        try:
            ...
            self.write(json_encode({
                "ok": True, "status": "ok",
                "project": body.get("project", ""),
                "confirmed_by": self.get_current_user() or "unknown",
            }))
        except Exception as e:
            logger.error(f"FieldConfirmInstallAPIHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class ShippingLoadsAPIHandler(BaseHandler):
    """GET/POST /api/shipping/loads — Shipping loads management."""
    required_roles = ["admin", "estimator", "shop"]
    ...
    def get(self):
        try:
            ...
            loads = []
            try:
                loads_path = os.path.join(DATA_DIR, "shipping_loads.json")
                if os.path.isfile(loads_path):
                    with open(loads_path) as f:
                        loads = json.load(f)
            except Exception:
                pass
            self.write(json_encode({"ok": True, "loads": loads}))
        except Exception as e:
            logger.error(f"ShippingLoadsAPIHandler GET error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))
        ...
        try:
            ...
            if os.path.isfile(loads_path):
                with open(loads_path) as f:
                    loads = json.load(f)
            new_load = {
                "id": "LOAD-" + str(uuid.uuid4())[:6].upper(),
                "destination": body.get("destination", ""),
                "job_code": body.get("job_code", ""),
                "items": body.get("items", []),
                "status": "building",
            ...
            logger.error(f"ShippingLoadsAPIHandler POST error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class ShippingItemsAPIHandler(BaseHandler):
    """POST /api/shipping/add-items and /api/shipping/remove-item."""
    required_roles = ["admin", "estimator", "shop"]
    ...
    def post(self):
        try:
            self.set_header("Content-Type", "application/json")
            body = json_decode(self.request.body)
            action = "add" if "add-items" in self.request.path else "remove"
            load_id = body.get("load_id", "")
            items = body.get("items", [])
            item_id = body.get("item_id", "")
            self.write(json_encode({
                ...
                "load_id": load_id,
                "items_affected": len(items) if action == "add" else (1 if item_id else 0),
            }))
        except Exception as e:
            logger.error(f"ShippingItemsAPIHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class ShippingTransitionAPIHandler(BaseHandler):
    """POST /api/shipping/transition — Transition shipping load status."""
    required_roles = ["admin", "estimator", "shop"]
    ...
    def post(self):
        try:
            ...
            new_status = body.get("status", "")
            self.write(json_encode({
                "ok": True, "load_id": load_id,
                "status": new_status,
                "transitioned_by": self.get_current_user() or "unknown",
            }))
        except Exception as e:
            logger.error(f"ShippingTransitionAPIHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class WorkOrderStatusConfigHandler(BaseHandler):
    """GET /api/work-orders/status-config — Work order status configuration."""
    required_roles = ["admin", "estimator", "shop"]
...
class ReportsExecutiveAPIHandler(BaseHandler):
    """GET /api/reports/executive — Executive KPI dashboard data."""
    required_roles = ["admin", "estimator"]
        ...
        try:
            ...
            try:
                ...
                if _os.path.isdir(SHOP_DRAWINGS_DIR):
                    from shop_drawings.work_orders import list_all_work_orders
                    all_wos = list_all_work_orders(SHOP_DRAWINGS_DIR)
                    kpis["open_work_orders"] = len(all_wos)
                    kpis["active_projects"] = len(set(wo.get("job_code", "") for wo in all_wos))
                projects_dir = os.path.join(DATA_DIR, "projects")
                if _os.path.isdir(projects_dir):
                    kpis["total_projects"] = len([d for d in _os.listdir(projects_dir) if _os.path.isdir(_os.path.join(projects_dir, d))])
            except Exception:
                pass
            self.write(json_encode({"ok": True, "kpis": kpis}))
        except Exception as e:
            logger.error(f"ReportsExecutiveAPIHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class WorkOrderReassignHandler(BaseHandler):
    """POST /api/work-orders/reassign — Reassign a work order."""
    required_roles = ["admin", "estimator"]
    ...
    def post(self):
        try:
            ...
            if not wo_id or not new_assignee:
                self.set_status(400)
                self.write(json_encode({"ok": False, "error": "work_order_id and assignee required"}))
                return
            self.write(json_encode({
                "ok": True, "work_order_id": wo_id,
                "assignee": new_assignee,
                "reassigned_by": self.get_current_user() or "unknown",
            }))
        except Exception as e:
            logger.error(f"WorkOrderReassignHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class WorkOrderBulkAssignHandler(BaseHandler):
    """POST /api/work-orders/bulk-assign — Bulk assign work orders."""
    required_roles = ["admin", "estimator"]
    ...
    def post(self):
        try:
            ...
            if not wo_ids or not assignee:
                self.set_status(400)
                self.write(json_encode({"ok": False, "error": "work_order_ids and assignee required"}))
                return
            self.write(json_encode({
                "ok": True, "assigned_count": len(wo_ids),
                "assignee": assignee,
                "assigned_by": self.get_current_user() or "unknown",
            }))
        except Exception as e:
            logger.error(f"WorkOrderBulkAssignHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class OperatorQueueHandler(BaseHandler):
    """GET /api/operator/queue — Operator work queue."""
    required_roles = ["admin", "estimator", "shop"]
        ...
        try:
            ...
            try:
                ...
                if _os.path.isdir(SHOP_DRAWINGS_DIR):
                    from shop_drawings.work_orders import list_all_work_orders, load_work_order
                    all_wos = list_all_work_orders(SHOP_DRAWINGS_DIR)
                    for wo_summary in all_wos:
                        if wo_summary.get("assigned_to", "").lower() == user.lower():
                            queue.append(wo_summary)
            except Exception:
                pass
            self.write(json_encode({"ok": True, "queue": queue, "operator": user}))
        except Exception as e:
            logger.error(f"OperatorQueueHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class AdminTestEmailHandler(BaseHandler):
    ...
    def post(self):
        try:
            ...
            if HAS_EMAIL_NOTIFICATIONS:
                ...
                self.write(json_encode({"ok": True, "status": "ok", "message": "Test email sent", "result": result}))
            else:
                self.write(json_encode({"ok": True, "status": "ok", "message": "Test email sent (email module not available, simulated)"}))
        except Exception as e:
            logger.error(f"AdminTestEmailHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))


class ProjectSyncFromDrawingHandler(BaseHandler):
    """POST /api/project/sync-from-drawing — Sync project data from drawing."""
    required_roles = ["admin", "estimator"]
    ...
    def post(self):
        try:
            ...
            if not job_code:
                ...
                self.write(json_encode({"ok": False, "error": "job_code required"}))
                return
            synced = {"bom_items": 0, "drawings": 0}
            try:
                import os as _os
                drawing_dir = _os.path.join(SHOP_DRAWINGS_DIR, job_code)
                if _os.path.isdir(drawing_dir):
                    config_path = _os.path.join(drawing_dir, "config.json")
                    if _os.path.isfile(config_path):
                        with open(config_path) as f:
                            config = json.load(f)
                        synced["drawings"] = len(config.get("drawings", []))
                        synced["bom_items"] = len(config.get("bom", []))
            except Exception:
                pass
            self.write(json_encode({"ok": True, "job_code": job_code, "synced": synced}))
        except Exception as e:
            logger.error(f"ProjectSyncFromDrawingHandler error: {e}", exc_info=True)
            self.set_status(500)
            self.write(json_encode({"ok": False, "error": str(e)}))
...
def get_routes():
    ...
    return [
        ...
        # ── New API Endpoints (stub handlers) ─────────────────
        (r"/api/shop-floor/summary",             ShopFloorSummaryHandler),
        (r"/api/work-station/scan",              WorkStationScanHandler),
        (r"/api/schedule/summary",               ScheduleSummaryHandler),
        (r"/api/schedule/capacity/usage",        ScheduleCapacityHandler),
        (r"/api/schedule/capacity/forecast",     ScheduleCapacityHandler),
        (r"/api/schedule/bottlenecks",           ScheduleBottlenecksHandler),
        (r"/api/schedule/overdue",               ScheduleOverdueHandler),
        (r"/api/field/summary",                  FieldSummaryAPIHandler),
        (r"/api/field/project-completion",       FieldProjectCompletionAPIHandler),
        (r"/api/field/punch-list",               FieldPunchListAPIHandler),
        (r"/api/field/punch-list/create",        FieldPunchListAPIHandler),
        (r"/api/field/installations",            FieldInstallationsAPIHandler),
        (r"/api/field/daily-reports",            FieldDailyReportsAPIHandler),
        (r"/api/field/daily-report",             FieldDailyReportsAPIHandler),
        (r"/api/field/confirm-install",          FieldConfirmInstallAPIHandler),
        (r"/api/shipping/loads",                 ShippingLoadsAPIHandler),
        (r"/api/shipping/create",                ShippingLoadsAPIHandler),
        (r"/api/shipping/add-items",             ShippingItemsAPIHandler),
        (r"/api/shipping/remove-item",           ShippingItemsAPIHandler),
        (r"/api/shipping/transition",            ShippingTransitionAPIHandler),
        (r"/api/work-orders/status-config",      WorkOrderStatusConfigHandler),
        (r"/api/reports/executive",              ReportsExecutiveAPIHandler),
