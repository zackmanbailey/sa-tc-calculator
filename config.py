...
class ShopDrawingConfig:
    ...
    above_grade_ft: float = 8.0
    cut_allowance_in: float = 6.0
    ...
    def from_bom_data(cls, bom_result: dict, project_info: dict = None) -> "ShopDrawingConfig":
        ...
        if geo:
            ...
            # Column-specific fields from BOM geometry
            cfg.above_grade_ft = geo.get("above_grade_ft", 8.0)
            cfg.cut_allowance_in = geo.get("cut_allowance_in", 6.0)

            # Rebar size — stored as "rebar_col" in geometry
            rebar = geo.get("rebar_col") or geo.get("col_rebar_size")
            if rebar:
                cfg.col_rebar_size = rebar

            # Reinforced toggle — may be in geometry or building-level
            if "col_reinforced" in geo:
                cfg.col_reinforced = bool(geo["col_reinforced"])
        ...
        # Also check building-level fields (SA Calculator puts these on the building object)
        buildings = bom_result.get("buildings", [])
        if buildings:
            b0 = buildings[0]
            if "reinforced" in b0:
                cfg.col_reinforced = bool(b0["reinforced"])
            if "rebar_col_size" in b0:
                cfg.col_rebar_size = b0["rebar_col_size"]
            if "above_grade_ft" in b0:
                cfg.above_grade_ft = float(b0["above_grade_ft"])
            if "cut_allowance_in" in b0:
                cfg.cut_allowance_in = float(b0["cut_allowance_in"])

        if project_info:
            cfg.job_code = project_info.get("job_code", "")
            cfg.project_name = project_info.get("project_name", "")
            cfg.customer_name = project_info.get("customer_name", cfg.customer_name)
