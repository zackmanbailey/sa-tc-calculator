...
# ─────────────────────────────────────────────────────────────────────────────
# QR CODE DATA GENERATION (BUG-001/002 FIX)
# ─────────────────────────────────────────────────────────────────────────────

def get_coil_qr_data(base_dir: str, coil_id: str,
                     app_base_url: str = "http://localhost:8888") -> dict:
    """Generate QR code data for a coil.

    When scanned, the QR code URL shows:
    - Coil details (type, gauge, grade, supplier)
    - Current stock level
    - Mill certificate link
    - Job assignments (which jobs are using this coil)
    - Transaction history
    """
    coil = get_coil(base_dir, coil_id)
    if not coil:
        return {"ok": False, "error": f"Coil {coil_id} not found"}

    # Build QR URL that opens the coil detail page
    qr_url = f"{app_base_url}/coil/{coil_id}"

    # Get mill certs for this coil
    certs = list_mill_certs(base_dir, coil_id=coil_id)

    # Get allocations (which jobs this coil is assigned to)
    allocs = list_allocations(base_dir, coil_id=coil_id)
    job_assignments = [
        {"job_code": a["job_code"], "quantity_lbs": a["quantity_lbs"],
         "status": a["status"]}
        for a in allocs if a.get("status") == "active"
    ]
    ...
    return {
        ...
        "status": coil.get("status", "active"),
        "mill_certs": certs,
        "job_assignments": job_assignments,
        "mill_cert_url": f"{app_base_url}/api/inventory/mill-cert/view?coil_id={coil_id}" if certs else None,
    }


def generate_coil_sticker_zpl(base_dir: str, coil_id: str,
                              app_base_url: str = "http://localhost:8888") -> str:
    """Generate ZPL commands for a coil inventory sticker.

    Designed for Zebra ZT411 Industrial Barcode Printer:
    - 203 DPI resolution
    - 4x6 thermal transfer labels
    - Wax/Resin ribbons

    Sticker includes: QR code, coil ID, gauge, grade, weight,
    supplier, heat number, mill cert indicator, job assignment.
    """
    coil = get_coil(base_dir, coil_id)
    if not coil:
        return ""

    qr_data = get_coil_qr_data(base_dir, coil_id, app_base_url)
    qr_url = qr_data.get("qr_url", f"{app_base_url}/coil/{coil_id}")

    label_w = 812   # 4" at 203 DPI
    label_h = 1218  # 6" at 203 DPI

    name = coil.get("name", "")[:40]
    gauge = coil.get("gauge", "")
    grade = coil.get("grade", "")
    supplier = coil.get("supplier", "")[:25]
    stock = f"{coil.get('stock_lbs', 0):,.0f} lbs"
    heat = coil.get("heat_num", "")
    width = f"{coil.get('width_in', 0)}\" wide"
    has_cert = "YES" if qr_data.get("mill_certs") else "NO"
    jobs = ", ".join([j["job_code"] for j in qr_data.get("job_assignments", [])[:3]]) or "UNASSIGNED"
    ...
    # Header bar
    zpl.append("^FO0,0^GB812,90,90,,^FS")
    zpl.append("^FO20,20^A0N,28,28^FR^FDTITANFORGE INVENTORY^FS")
    zpl.append(f"^FO550,20^A0N,22,22^FR^FD{print_date}^FS")
    zpl.append("^FO20,55^A0N,20,20^FR^FDCOIL TRACKING LABEL^FS")

    # Gold accent line
    zpl.append("^FO0,92^GB812,4,4,,^FS")

    # QR Code (left side, links to coil detail page)
    zpl.append(f"^FO30,120^BQN,2,7^FDMM,{qr_url}^FS")

    # Coil ID (large, right of QR)
    zpl.append(f"^FO350,130^A0N,56,56^FD{coil_id}^FS")

    # Description
    zpl.append(f"^FO350,195^A0N,24,24^FD{name}^FS")

    # Gauge and Grade
    zpl.append(f"^FO350,230^A0N,28,28^FD{gauge}  |  {grade}^FS")

    # Width
    zpl.append(f"^FO350,270^A0N,22,22^FD{width}^FS")

    # Divider
    zpl.append("^FO20,420^GB772,2,2,,^FS")

    # Info grid
    gy = 440
    zpl.append(f"^FO30,{gy}^A0N,18,18^FDSUPPLIER^FS")
    zpl.append(f"^FO30,{gy+25}^A0N,26,26^FD{supplier}^FS")
    zpl.append(f"^FO350,{gy}^A0N,18,18^FDHEAT #^FS")
    zpl.append(f"^FO350,{gy+25}^A0N,26,26^FD{heat}^FS")
    zpl.append(f"^FO600,{gy}^A0N,18,18^FDSTOCK^FS")
    zpl.append(f"^FO600,{gy+25}^A0N,26,26^FD{stock}^FS")

    # Divider
    gy2 = gy + 70
    zpl.append(f"^FO20,{gy2}^GB772,2,2,,^FS")

    # Mill cert and job info
    gy3 = gy2 + 15
    zpl.append(f"^FO30,{gy3}^A0N,20,20^FDMILL CERT: {has_cert}^FS")
    zpl.append(f"^FO350,{gy3}^A0N,20,20^FDJOB(S): {jobs}^FS")

    # Scan instruction
    zpl.append(f"^FO30,{gy3+40}^A0N,18,18^FDSCAN QR FOR MILL CERT & JOB DETAILS^FS")

    # Footer
    zpl.append(f"^FO0,{label_h-40}^GB812,40,40,,^FS")
    zpl.append(f"^FO120,{label_h-30}^A0N,18,18^FR^FDStructures America | Conroe TX | TitanForge Inventory^FS")
...
# ─────────────────────────────────────────────────────────────────────────────
# AUTO-DEDUCT FROM FABRICATION (BUG-001/002 FIX)
# ─────────────────────────────────────────────────────────────────────────────

# Material usage estimates per component type (lbs per unit)
MATERIAL_USAGE = {
    "column": {"coil_type": "c_section_23", "lbs_per_unit": 450},
    "rafter": {"coil_type": "c_section_23", "lbs_per_unit": 380},
    "purlin": {"coil_type": "z_purlin_20", "lbs_per_unit": 120},
    "sag_rod": {"coil_type": "angle_4_16ga", "lbs_per_unit": 8},
    "strap": {"coil_type": "strap_16ga", "lbs_per_unit": 3},
    "endcap": {"coil_type": "c_section_23", "lbs_per_unit": 25},
    "roofing": {"coil_type": "spartan_rib_48", "lbs_per_unit": 200},
}


def auto_deduct_for_fabrication(base_dir: str, component_type: str,
                                quantity: int, job_code: str,
                                work_order_ref: str = "",
                                deducted_by: str = "system") -> dict:
    """Automatically deduct inventory when a fabrication item starts.

    Called by the QR scan start workflow. Looks up the material usage
    for the component type and deducts from the appropriate coil.

    Returns dict with ok, transaction details, and any alerts triggered.
    """
    usage = MATERIAL_USAGE.get(component_type)
    if not usage:
        return {"ok": False, "error": f"Unknown component type: {component_type}"}

    coil_type = usage["coil_type"]
    lbs_needed = usage["lbs_per_unit"] * quantity

    # Check if coil exists and has enough stock
    coil = get_coil(base_dir, coil_type)
    if not coil:
        return {"ok": False, "error": f"Coil type {coil_type} not found in inventory",
                "coil_type": coil_type, "lbs_needed": lbs_needed}

    available = coil.get("stock_lbs", 0) - coil.get("committed_lbs", 0)
    if available < lbs_needed:
        # Don't block fabrication — just warn
        return {
            "ok": True,
            "warning": f"Low stock: {coil_type} has {available:.0f} lbs available, need {lbs_needed:.0f} lbs",
            "coil_type": coil_type,
    ...
    result = consume_stock(
        base_dir, coil_type, lbs_needed,
        job_code=job_code,
        work_order_ref=work_order_ref,
        notes=f"Auto-deduct for {component_type} x{quantity} (fabrication start)",
        consumed_by=deducted_by,
    )
...
# ─────────────────────────────────────────────────────────────────────────────
# PURCHASE ORDER MANAGEMENT (BUG-001/002 FIX)
# ─────────────────────────────────────────────────────────────────────────────

PO_STATUSES = ["draft", "submitted", "confirmed", "partially_received", "received", "cancelled"]

def _load_purchase_orders(base_dir: str) -> list:
    d = _inv_dir(base_dir)
    return _load_json(os.path.join(d, "purchase_orders.json")) or []

def _save_purchase_orders(base_dir: str, pos: list):
    d = _inv_dir(base_dir)
    _save_json(os.path.join(d, "purchase_orders.json"), pos)


def create_purchase_order(base_dir: str, supplier: str, items: list,
                         notes: str = "", created_by: str = "") -> dict:
    """Create a new purchase order.

    items: [{"coil_type": "...", "quantity_lbs": ..., "price_per_lb": ...}, ...]
    """
    pos = _load_purchase_orders(base_dir)
    now = datetime.datetime.now()

    po = {
        "po_id": f"PO-{now.strftime('%Y%m%d')}-{uuid.uuid4().hex[:4].upper()}",
        "supplier": supplier,
        "status": "draft",
        "items": items,
        "total_lbs": sum(i.get("quantity_lbs", 0) for i in items),
        "total_cost": sum(i.get("quantity_lbs", 0) * i.get("price_per_lb", 0) for i in items),
        "received_lbs": 0,
        "notes": notes,
        "created_by": created_by,
    ...
    pos.insert(0, po)
    _save_purchase_orders(base_dir, pos)
    return po


def list_purchase_orders(base_dir: str, status: str = "",
                        supplier: str = "") -> list:
    ...
    for po in pos:
        if status and po.get("status") != status:
            continue
        if supplier and supplier.lower() not in po.get("supplier", "").lower():
            continue
        results.append(po)
    return results


def update_purchase_order(base_dir: str, po_id: str, **kwargs) -> dict:
    """Update a purchase order's fields."""
    pos = _load_purchase_orders(base_dir)
    for po in pos:
        if po["po_id"] == po_id:
            for k, v in kwargs.items():
...
def receive_against_po(base_dir: str, po_id: str, coil_id: str,
                      quantity_lbs: float, heat_number: str = "",
                      bol_number: str = "", condition_notes: str = "",
                      received_by: str = "") -> dict:
    """Receive stock against a specific purchase order.

    Combines PO tracking with the regular receive_stock flow.
    """
    pos = _load_purchase_orders(base_dir)
    po = None
    for p in pos:
    ...
    if po["status"] in ("cancelled", "received"):
        raise ValueError(f"PO {po_id} is {po['status']} and cannot receive")

    # Do the actual stock receive
    result = receive_stock(
        base_dir, coil_id, quantity_lbs,
        po_number=po_id,
        bol_number=bol_number,
    ...
    # Update PO tracking
    po["received_lbs"] = po.get("received_lbs", 0) + quantity_lbs
    now = datetime.datetime.now().isoformat()
    po["received_at"] = now

    if po["received_lbs"] >= po["total_lbs"]:
        po["status"] = "received"
    else:
    ...
    return {
        ...
        "po_status": po["status"],
        "po_received_lbs": po["received_lbs"],
        "po_total_lbs": po["total_lbs"],
    }
